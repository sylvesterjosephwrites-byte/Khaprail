...
**Chat widget** (`src/components/shared/ai-chat-widget.tsx`, mounted globally in `SiteLayout`, every page) — a `bg-primary` circular floating button, bottom-**left**, specifically to avoid the existing WhatsApp bubble (bottom-right). On mobile, both floating buttons additionally have to clear the bottom tab bar (batch: mobile navigation pattern) — the chat button sits at `bottom-[calc(env(safe-area-inset-bottom)+5.5rem)]` on mobile vs. a normal near-zero offset at `lg:`, and the open panel is offset similarly (`bottom-[calc(env(safe-area-inset-bottom)+9.5rem)]` mobile / `lg:bottom-24`). The open panel: message bubbles (user right-aligned `bg-primary`, assistant left-aligned `bg-muted`, rate-limit/error notices in `bg-destructive/10`), a streaming assistant bubble that fills in as text chunks arrive (no long blocking wait), and an input whose placeholder changes based on real page context ("Ask about this product…" / "Ask about this category…" / "Ask about Khaprail Tiles…") via the same `useMatch` pattern `FloatingWhatsAppButton` already uses for its per-product message. A small "AI-generated, may be imperfect" label sits in the panel header at all times, not just next to individual messages.
